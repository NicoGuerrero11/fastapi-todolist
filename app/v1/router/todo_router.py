...
async def get_user_todos(user_uid: UUID, session: AsyncSession = Depends(get_session)):
    todo = await ts.get_user_todos(user_uid ,session )
    return todo

@todo_router.get("/todos/{todo_id}", response_model=List[TodoRead])
async def get_single_todo(todo_id: UUID, session: AsyncSession = Depends(get_session)):
...
async def create_todo(todo_data: TodoCreate, session: AsyncSession = Depends(get_session)):
    new_todo = await ts.create_todo(todo_data, session)
    return new_todo

@todo_router.patch("/todos/{todo_id}", response_model=TodoRead)
async def update_todo(todo_id: UUID, update_data:TodoUpdate, session: AsyncSession = Depends(get_session)):
...
async def delete_todo(todo_id: UUID, session: AsyncSession = Depends(get_session)):
